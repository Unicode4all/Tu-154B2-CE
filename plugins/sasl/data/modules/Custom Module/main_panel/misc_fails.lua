	...
	if MASTER then	
		...
		if FAIL > 0 then
			...
			if fail_counter > check_time then
				...
				if get(S1_fail) ~= 1 then set(S1_fail, bool2int(math.random() < 0.00001 * FAIL * 0.3) * 1) end
				if get(S2_fail) ~= 1 then set(S2_fail, bool2int(math.random() < 0.00001 * FAIL * 0.3) * 1) end
				if get(S3_fail) ~= 1 then set(S3_fail, bool2int(math.random() < 0.00001 * FAIL * 0.3) * 1) end
